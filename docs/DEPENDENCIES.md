...
权威说明与 JSON 示例：[SHARED_DMS_BROWSER.md](SHARED_DMS_BROWSER.md)

| 计划 | 禁刷约起 | 任务 | schedule id |
|------|----------|------|-------------|
| 08:30 | 08:27 | 区域报表 | `district-form` |
| 09:00 | 08:57 | VIP 保养提醒 | `vip-alert` |
| 10:00 | 09:57 | 事故车上午 | `accident-morning` |
| 17:00 | 16:57 | 事故车下午 | `accident-evening` |

规则摘要：`pre_minutes=3` 提前禁刷 → 爬取登记期间持续禁刷 → `unregister` 后恢复；到点未开跑则最多再等 `await_start_minutes=45`。
